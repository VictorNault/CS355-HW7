...
int f_mkdir(const char *pathname, char *mode) {
    ...
    strcpy(new_dir_entry->name, "\0\0\0\0\0\0\0\0");
    ...
    if(files_in_parent >= 15){
        ...
        if (dum.first_FAT_idx != 0) set_file_size(&dum,parent_dir->size);
...
int f_mkfile(const char *pathname, char *mode) {
    ...
    for (int i = 0; i < 480; i++){
        new_file->data_in_first_block[i] = '\0';
    }
...
file_handle *f_open(const char *pathname, const int mode){
    ...
    for(int i = 0; i < token_length; i++){
        ...
        if(status == EXIT_FAILURE && i < token_length - 1){
            ...
            int good_file = f_mkfile(pathname,"e");
            if (good_file == E_BAD_NAME){
            for(int i = 0; i < token_length; i++){
                free(tokens[i]);
            }
            free(tokens);
...
dir_handle *f_opendir(const char *pathname){
    ...
    //***check if open files already contains the same file, then don't open
    for(int i = 0; i < NUM_OPEN_FILES; i++){
        if(open_files[i] && open_files[i]->first_FAT_idx == file_e->first_FAT_idx && file_e->first_FAT_idx != 0){
            //printf("File %s already open, exiting f_open...\n", file_e->name);
            f_error = E_FILE_ALREADY_OPEN;
            for(int i = 0; i < token_length; i++){
                free(tokens[i]);
...
int f_rmdir(const char *pathname){
    ...
    for (int i = 0; i < files_in_parent_dir; i++) {
        ...
        if (strcmp(name, curr_dir_entry.name) == 0) {
            ...
            to_delete_dir_entry = curr_dir_entry;
            block_to_modify_fat_index = curr_fat_index;
            if (curr_dir_entry.is_directory == FALSE) {
                f_error = E_NOT_DIR;
                return EXIT_FAILURE;
            }
        }
    ...
    // --- new code for recursive deletion ---
    
    // +1 for null char
    int recursive_del_dir_block = to_delete_dir_entry.first_FAT_idx;
    fseek(disk, find_offset(recursive_del_dir_block), SEEK_SET);
    dir_header *recursive_del_parent_dir = malloc(sizeof(dir_header));
    fread(recursive_del_parent_dir, BLOCK_SIZE, 1, disk);


    // get correct dir entry

    // look for file to delete
    int files_in_dir_to_delete = (curr_block_ar[to_delete_block_ar_index].size - FILE_HEADER_BYTES) / DIR_ENTRY_BYTES;
    // initialize to data after header
    int recursive_del_possible_files_left_in_block = FILE_AFTER_HEADER_BYTES / DIR_ENTRY_BYTES;
    dir_entry recursive_del_curr_block_ar[BLOCK_SIZE / DIR_ENTRY_BYTES];
    for (int i = 0; i < recursive_del_possible_files_left_in_block; i++) {
        recursive_del_curr_block_ar[i] = recursive_del_parent_dir->data_in_first_block[i];
    }
    ...
    for (int i = 2; i < files_in_dir_to_delete; i++) {
        ...
        if (recursive_del_possible_files_left_in_block == 0) {
            ...
            recursive_del_ar_counter = 0;
            // get next block
            fseek(disk, find_offset(recursive_del_curr_fat_entry.next), SEEK_SET);
            fread(recursive_del_curr_block_ar, BLOCK_SIZE, 1, disk);
            recursive_del_curr_fat_index = recursive_del_curr_fat_entry.next;
            recursive_del_curr_fat_entry = fat_table[recursive_del_curr_fat_entry.next];
        }
